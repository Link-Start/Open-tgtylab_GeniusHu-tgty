...
from reverselab_mcp.config import REVERSE_ROOT, SERVER_NAME, HOST_PYTHON_EXE, HOST_PYTHON_SOURCE
from reverselab_mcp.errors import classify_error
...
@mcp.tool()
def list_samples(limit: int = 200, subdir: str = "", include_hash: bool = False) -> dict[str, Any]:
    """列出 samples 下样本，可按子目录过滤。include_hash=True 时计算 SHA-256（大目录会慢）。"""
    return _safe_call(sample_crud.list_samples, limit, subdir, include_hash)
...
@mcp.tool()
def server_healthcheck() -> dict[str, Any]:
    """检查 MCP server 和所有外部工具/Python 库的健康状态。启动时调用一次。"""
    import importlib.util

    from reverselab_mcp.config import (
        HOST_PYTHON_EXE, HOST_PYTHON_SOURCE,
        GHIDRA_ROOT, CUTTER_ROOT, DIE_ROOT,
        RZ_BIN_EXE, DIEC_EXE, PE_BEAR_EXE,
        PROCMON_ROOT, X64DBG_ROOT, ADB_EXE,
    )
    from reverselab_mcp.runner import get_launched_status

    # External tools
    tools = {}
    tool_checks = {
        "Ghidra": GHIDRA_ROOT / "support" / "analyzeHeadless.bat",
        "Cutter/Rizin": CUTTER_ROOT / "rizin.exe",
        "rz-bin": RZ_BIN_EXE,
        "DiE CLI": DIEC_EXE,
        "PE-bear": PE_BEAR_EXE,
        "Procmon": PROCMON_ROOT / "Procmon64.exe",
        "x64dbg": X64DBG_ROOT / "x64" / "x64dbg.exe",
        "ADB": ADB_EXE,
    }
    for name, path in tool_checks.items():
        tools[name] = {"available": path.exists(), "path": str(path)}

    # Python RE libraries
    re_libs = {}
    for lib in ["lief", "frida", "angr", "capstone", "keystone", "unicorn"]:
        re_libs[lib] = importlib.util.find_spec(lib) is not None

    # Knowledge base
    from reverselab_mcp.config import REVERSE_ROOT
    kb_dir = REVERSE_ROOT / "kb"
    kb_boards = []
    if kb_dir.exists():
        for board in sorted(kb_dir.iterdir()):
            if board.is_dir() and (board / "techniques").exists():
                tech_count = len(list((board / "techniques").rglob("*.md")))
                kb_boards.append({"board": board.name, "technique_files": tech_count})

    available_tools = sum(1 for t in tools.values() if t["available"])
    installed_libs = sum(1 for v in re_libs.values() if v)

    return {
        "status": "ok" if available_tools > 0 and installed_libs >= 4 else "degraded",
        "host_python": {
            "path": str(HOST_PYTHON_EXE),
            "source": HOST_PYTHON_SOURCE,
            "is_venv": (HOST_PYTHON_EXE.parent / "pyvenv.cfg").exists(),
        },
        "external_tools": tools,
        "external_tools_available": f"{available_tools}/{len(tools)}",
        "python_re_libs": re_libs,
        "python_re_libs_installed": f"{installed_libs}/{len(re_libs)}",
        "knowledge_base": kb_boards,
        "launched_processes": get_launched_status(),
    }
