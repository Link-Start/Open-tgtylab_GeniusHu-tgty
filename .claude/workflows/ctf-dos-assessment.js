...
const targets = (typeof args === 'object' && args?.targets) ? args.targets : [
  { host: `www.${domain}`, ip: null, stack: 'unknown', cdn: false, waf: false, h2: false },
]
const DNS_NS = (typeof args === 'object' && args?.dns_ns) ? args.dns_ns : [`ns1.${domain}`, `ns2.${domain}`]
const geofenced = [`cas.${domain}`, `idp.${domain}`, `vpn.${domain}`, `webvpn.${domain}`, `oa.${domain}`]
const unprobed = [`jwgl.${domain}`, `jwc.${domain}`, `xxgk.${domain}`]
...
const DNS_EMAIL_PROBE = [
  ...
  `- Test authoritative NS response times (dig @${DNS_NS[0]} ${domain}, measure RTT)`,
...
const DEEP_PROBE_PROMPT = [
  ...
  '- Check if proxy Via header is exploitable',
  '',
  '### 3. QQ Mail (mail)',
  ...
  '',
  `### 4. Internal API (sqh.${domain} from GitHub)`,
  `- Probe http://sqh.${domain}/api (may be reachable from different network)`,
